// Package fastenstore provides Go bindings for libfasten_core.
//
...
//
//	CGO_CFLAGS="-I/path/to/fasten/fasten-core/include"
//	CGO_LDFLAGS="-L/path/to/target/release -lfasten_core -Wl,-rpath,..."
//
// Example:
//
...
func takeError(raw *C.char) error {
	if raw == nil {
		return errors.New("fasten-core: unknown error")
	}
	msg := C.GoString(raw)
	C.fasten_store_free_str(raw)
	return fmt.Errorf("fasten-core: %s", msg)
}
