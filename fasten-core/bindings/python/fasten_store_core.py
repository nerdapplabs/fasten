"""
fasten_core — Python binding for libfasten_core.
...
Build lib: cd fasten/fasten-core && cargo build --release --features all

Usage:
    from fasten_core import FastenStore
...
def _lib_name() -> str:
    ...
    if system == "Darwin":
        return "libfasten_core.dylib"
    if system == "Windows":
        return "fasten_core.dll"
    return "libfasten_core.so"
...
def _find_lib() -> str:
    ...
    raise ImportError(
        ...
        "`cargo build --release --features all` inside fasten/fasten-core/."
    )
