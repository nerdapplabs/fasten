...
| Node.js  | `node:24-alpine`     | audit · sys          | [`node/`](node/)     |
...
| Node   | 24      | Matches `js/package.json` engines `>=24` |
...
These tests are wired into CI: a GitHub Actions workflow runs
`make test` in a matrix, each language as a parallel job.
